...
class BrowserDelegationManager:
    ...
    def _build_browser_task(self, *, intent: str, start_url: str, initial_links: list[str], plan_task: str) -> str:
        ...
        return (
            ...
            "- Use the initial links list as hints, but stay on the same registrable domain as the start URL.\n"
            "- Do not navigate to external search engines (Google, DuckDuckGo, Bing, Yahoo) or unrelated domains.\n"
            "- Stay anchored to URLs you actually opened in this session. Do not mention or reason about a different domain.\n"
            "- If navigation accidentally leaves the start domain, immediately go back and continue on-domain.\n"
            ...
            "- If the start page is inaccessible, try one same-domain recovery path (homepage or close sibling) and then stop with blocked/unknown if still inaccessible.\n"
            "- CAPTCHAs are not automatically solved in this environment.\n"
            "- If captcha appears anywhere, stop and report captcha. Do not claim that CAPTCHA was solved automatically.\n"
            ...
            "- Example: if the homepage is accessible but the right section is unclear, try menu/footer/site-search paths like procurement, tenders, opportunities, or vendor pages before giving up.\n"
